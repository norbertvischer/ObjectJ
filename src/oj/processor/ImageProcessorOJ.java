...
//    }
	public void propagateScale(int index) {
		...
		GenericDialog gd = new GenericDialog("Scale Linked Images");
		gd.addMessage("Current Image:\n " + title + "\n  ");
		gd.addNumericField("New Scale", scale, 3, 9, "px/unit");
		gd.addStringField("Unit:", unit);
		gd.addCheckbox("Propagate to all linked images", false);
		gd.setOKLabel("Change Scale");
		gd.showDialog();
		if (gd.wasOKed()) {
			...
			for (int im = 0; im < nImages; im++) {
				...
				if (scaleAll || im == index) 
				{
					...
					if (isTiff) {
						if (shouldChange) {//allow 0.1% difference
							if (imj.isFileExists()) {
