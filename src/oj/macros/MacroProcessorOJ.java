...
public class MacroProcessorOJ {
	...
	/**
	 * For setting markers into other than current window
	 */
	public void setTarget(String arg) {
		...
		if (parts[0].equals("frontimage")) {
			...
			targetImageOJ = null;
			if (false) {
				OJ.getImageProcessor().updateImagesProperties();//dangerous, removed on 23.2.2017
			}
...
//    }
	...
	public void orderObjectsInZ(int whichImage) {
		ImagesOJ images = OJ.getData().getImages();
		CellsOJ cells = OJ.getData().getCells();//ArrayList
	...
	public void renumberObjects(int[] from, int[] to) {//zero based
		CellsOJ cells = OJ.getData().getCells();

		CellOJ[] tmpCells1 = cells.cellsToArray();
		CellOJ[] tmpCells2 = cells.cellsToArray();
//check crosss-image conflict
		for (int jj = 0; jj < from.length; jj++) {
			if (from[jj] < 0 || from[jj] >= from.length) {
				ImageJAccessOJ.InterpreterAccess.interpError("Object " + (from[jj] + 1) + ":  Out of range");
				return;
			}
			String name1 = cells.getCellByIndex(from[jj]).getImageName();
			String name2 = cells.getCellByIndex(to[jj]).getImageName();
			if (!name1.equals(name2)) {
				ImageJAccessOJ.InterpreterAccess.interpError("Cannot re-order objects across different images (from=" + from[jj] + ", to=" + to[jj] + ")");
				return;
			}
		}
		for (int jj = 0; jj < from.length; jj++) {
			tmpCells2[to[jj]] = tmpCells1[from[jj]];
		}
		for (int jj = 0; jj < cells.getCellsCount(); jj++) {
			cells.setCell(jj, tmpCells2[jj]);
		}
	}
...
//
	...
	/**
	 * Finds closest object point in active image and slice. The point must
	 * belong to an item of ytemName (wildcard allowed) and its distance to
	 * (x,y) must be within maxDistance (pixels). If found, the corresponding
	 * object and item is selected and point number (1-based) is returned. If
	 * not found, 0 is returned and no object is selected.
	 */
	public int findClosestPoint(int x, int y, int maxdistance, String ytemName) {
		WildcardMatchOJ wm = new WildcardMatchOJ();
		wm.setCaseSensitive(false);
		CellsOJ cells = OJ.getData().getCells();
		ImagePlus imp = IJ.getImage();
		...
		int slc = imp.getCurrentSlice();
		String imgName = IJ.getImage().getTitle();
		int firstIndex = cells.getFirstCellOnImage(imgName);
		int lastIndex = cells.getLastCellOnImage(imgName);
		int foundCell = -1;
		int foundItem = -1;
		int foundPoint = -1;
		double minDist = maxdistance;
		for (int cellNo = firstIndex; cellNo <= lastIndex; cellNo++) {
			CellOJ cell = cells.getCellByIndex(cellNo);
			int nYtems = cell.getYtemsCount();
			for (int ytemNo = 0; ytemNo < nYtems; ytemNo++) {
				YtemOJ ytem = cell.getYtemByIndex(ytemNo);
				int thisZ = Math.round(ytem.getLocation(0).z);
				if (thisZ != slc) {
					continue;
				}
				if (wm.match(ytem.getYtemDef(), ytemName)) {
					int[] xx = ytem.toXArray();
					int[] yy = ytem.toYArray();
					for (int pointNo = 0; pointNo < xx.length; pointNo++) {
						double distance = Math.sqrt((xx[pointNo] - x) * (xx[pointNo] - x) + (yy[pointNo] - y) * (yy[pointNo] - y));
						if (distance <= minDist) {
							minDist = distance;
							foundCell = cellNo;
							foundItem = ytemNo;
		...
		if (foundPoint >= 0) {
			CellOJ cell = cells.getCellByIndex(foundCell);
			OJ.getDataProcessor().selectCell(foundCell);
			cell.selectYtem(foundItem);
			return (foundPoint + 1);
		}
		OJ.getDataProcessor().selectCell(-1);//unselect
		return 0;
	}
	...
	// can init several space or comma separated names
	public void initColumn(String columnNames, boolean isUnlinked, boolean isTextMode) {//10.5.2010synchronized
		if (columnNames.contains("?") || columnNames.contains("\t") || columnNames.contains("*")) {
	...
	public void deleteColumn(String columnNamePattern) {
		String[] parts = Tools.split(columnNamePattern, " ,");
		for (int nn = 0; nn < parts.length; nn++) {
			String thisPattern = parts[nn];
			...
			for (int colIndex = columnNames.length - 1; colIndex >= 0; colIndex--) {
				...
				if (wm.match(thisName, thisPattern)) {
...
//		}
		...
		if (canvas != null) {
			imp.getCanvas().setCursor(-1, -1, -1, -1);//23.2.2017
		}
	}
...
//	IJ.showStatus("linkNo=" + linkNo);
//	if (linkNo >= 0) {
//	    int firstCell = images.getImageByIndex(linkNo).getFirstCell();
//	    int lastCell = images.getImageByIndex(linkNo).getLastCell();
//	    if (firstCell < 0 || lastCell < 0) {
//		return;
//	    }
//
//	    CellsOJ cells = OJ.getData().getCells();
////			for (int jj = firstCell; jj <= lastCell; jj++) {
////				CellOJ cell = cells.getCellByIndex(jj);
////				for(int ytem = 0; ytem < cell.)
//
//	}
//
...
//	}
	...
	public void ojvUndoPush() {
		VertexCalculatorOJ vc = OJ.getVertexCalculator();
		while (vc.size() > vc.previousSize) {
			Object dummy = vc.pop();
		}
	}
