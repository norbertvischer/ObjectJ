			...
			for (int i = 1; i < macros.length; i++) {
				...
				if ((macro_name != null) && (!macro_name.startsWith("Unused"))) {
					...
					if (ss.contains("tool-")) {
						...
						int ampersandPos = macro_ext_name.indexOf("[&");//true= both number keys work
						String savedName = macro_ext_name;
						macro_ext_name = macro_ext_name.replace("[&", "[");						
						int shortcutIndex = MacroSetOJ.extractMacroShortcut(macro_ext_name);
						item.setShortcut(shortcutIndex);
						Hashtable macroShortcuts = Menus.getMacroShortcuts();
						macroShortcuts.put(new Integer(shortcutIndex), "^" + macro_ext_name); 
						if (ampersandPos > 0) {
							macro_ext_name = savedName.replace("[&", "[N");
						}
						shortcutIndex = MacroSetOJ.extractMacroShortcut(macro_ext_name);
						macroShortcuts = Menus.getShortcuts();
						macroShortcuts.put(new Integer(shortcutIndex), "^" + macro_ext_name);
						macroItems.add(item);
					}
				}
