...
public class ColumnsOJ extends BaseAdapterOJ implements StatisticsChangedListenerOJ {
	...
	//pattern can contain several patterns separated by comma or space //2017
	public ArrayList getColumnsByWildcard(String pattern) {

		HashSet<ColumnOJ> hashSet = new HashSet<ColumnOJ>();
		String[] patterns = Tools.split(pattern, " ,");
		...
		for (int kk = 0; kk < patterns.length; kk++) {
			...
			for (int jj = 0; jj < getAllColumnsCount(); jj++) {
				...
				if (wm.match(thisCol.getName(), patterns[kk])) {
					hashSet.add(thisCol);
					//matchingColumns.add(thisCol);
				}
			}
		}
		ArrayList<ColumnOJ> matchingColumns = new ArrayList<ColumnOJ>(hashSet);
